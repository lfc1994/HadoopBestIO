...
public class TestCodec extends TestCase {
	...
	public void testCreation() throws Exception {
		...
		assertEquals("rs_local", Codec.getCodec("rs_local").id);
		...
		assertEquals("rs_local", codecs.get(3).id);
		assertEquals(6, codecs.get(3).stripeLength);
		assertEquals(2, codecs.get(3).parityLength);
		assertEquals(50, codecs.get(3).priority);
		assertEquals("/raid_rslocal", codecs.get(3).parityDirectory);
		assertEquals(false, codecs.get(3).simulateBlockFix);
		...
		assertTrue(codecs.get(3).createErasureCode(conf) instanceof LocalRS);
	}
