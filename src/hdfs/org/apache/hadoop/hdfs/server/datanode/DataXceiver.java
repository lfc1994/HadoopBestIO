...
class DataXceiver implements Runnable, FSConstants {
		...
		try {
			...
			switch (op) {
				case DataTransferProtocol.OP_READ_BLOCK:
					...
					break;
				//for version 24
				case DataTransferProtocol.OP_COMPUTE_BLOCK:
					computeBlock(in, versionAndOpcode);
					break;
	...
	/**
	 *
	 * @param in The packet header info stream
	 * @throws IOException
	 */
	private void computeBlock(DataInputStream in,
							  VersionAndOpcode versionAndOpcode) throws IOException{

		// 协议头读写器,块计算操作和块读取操作头是相同的
		ComputeBlockHeader header = new ComputeBlockHeader(versionAndOpcode);
		header.readFields(in);

		// 读取协议头中的相关信息
		int namespaceId = header.getNamespaceId();
		long blockId = header.getBlockId();
		byte blockInStripe = header.getBlockInStripe();
		...
		ComputeBlockSender sender = null;
		try {
			sender = new ComputeBlockSender(namespaceId,block,blockInStripe,datanode,out);

			// 发送成功状态码
			out.writeShort(DataTransferProtocol.OP_STATUS_SUCCESS);
			//计算并发送数据
			sender.computeAndSend();
