...
public class BlockSender implements java.io.Closeable, FSConstants {
	...
	private int sendChunks(ByteBuffer pkt, int maxChunks, OutputStream out)
		...
		// 多个chunk in one packet 一次write,一个包
		...
		pkt.putLong(offset);//数据包中数据在块中的偏移量
		pkt.putLong(seqno);//包的序列号
		pkt.put((byte) ((offset + len >= endOffset) ? 1 : 0));//是否是最后一个包
		//why no ByteBuf.putBoolean()?
		pkt.putInt(len);//包中源数据的长度
		...
		if (blockInPosition < 0) {
			...
			try {
				...
				if (memoizedBlock.hasBlockChanged(len)) {//文件发生变化,读写竞争
	...
	public long sendBlock(DataOutputStream out, OutputStream baseStream,
		...
		//out是baseStream的套接流,baseStream是向socket输出的流
		if (out == null) throw new IOException("out stream is null");
